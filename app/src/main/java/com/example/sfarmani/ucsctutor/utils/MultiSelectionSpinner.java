 ...
 * Copied from https://trinitytuts.com/tips/multiselect-spinner-item-in-android/
 */
public class MultiSelectionSpinner extends Spinner implements OnMultiChoiceClickListener {
    ...
    public ArrayList<Boolean> getSelectedItemsAsBoolean() {
        ArrayList<Boolean> selectedDays = new ArrayList<>(7);
        boolean foundOne;

        for (int i = 0; i < _items.length; ++i) {
            foundOne = mSelection[i];
            selectedDays.add(i, foundOne);
        }
        return selectedDays;
    }
}
